...
class EvaluationSchema(BaseModel):
    feedback: str = Field(
        description="Constructive detailed feedback for the essay.")
    score: int = Field(
        description="Score out of 10 based on content, structure, and language.",
        ge=0, le=10)
...
class UPSCState(TypedDict):
    ...
    indivitual_scores: Annotated[list[int], add]
    avg_score: float


def evaluate_language(state: UPSCState) -> dict[str, str | list[int]]:
    prompt = f'Evaluate the language quality of the following essay and provide a feedback and assign a score out of 10 \n {state["essay"]}'
    output = structured_model.invoke(prompt)
    return {
        "language_feedback": output.feedback,
        "indivitual_scores": [output.score]
    }


def evaluate_analysis(state: UPSCState) -> dict[str, str | list[int]]:
...
def evaluate_clarity(state: UPSCState) -> dict[str, str | list[int]]:
...
def final_evaluation(state: UPSCState) -> dict[str, str | float]:
    prompt = f'Based on the following feedbacks create a summarized feedback \n language feedback - {state["language_feedback"]} \n depth of analysis feedback - {state["analysis_feedback"]} \n clarity of thought feedback - {state["clarity_feedback"]}'
    overall_feedback = model.invoke(prompt).content
    avg_score = sum(state['indivitual_scores'])/len(state['indivitual_scores'])

    return {
        "overall_feedback": overall_feedback,  # type: ignore
